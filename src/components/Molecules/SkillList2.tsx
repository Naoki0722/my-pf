  ...
  return (
    <div className='relative mb-24'>
      ...
      <div className='flex flex-wrap justify-between bg-white rounded-[4.5rem] sm:rounded-2xl sm:w-11/12 sm:mx-auto'>
        <SkillDetail language='Docker' icon={faDocker} width='w-2/5 sm:w-1/6'>
          {`経験年数 1.5年\n本業、副業含めサーバーサイド側の言語はDockerで開発。 AWSではECS、GCPではAppEngineやCloudRunを利用し、Dockerに対する知識を日々得ております。`}
        </SkillDetail>
        <SkillDetail language='GCP' icon={faGoogle} width='w-2/5 sm:w-1/6'>
          {`経験年数 1年\nサービスの保守運用。現在も携わっています。`}
        </SkillDetail>
        <SkillDetail language='Terraform' icon={faT} width='w-2/5 sm:w-1/6'>
          {`経験年数 1年\nGCPの環境設定を構築、設定変更も実施。`}
        </SkillDetail>
        <SkillDetail language='AWS' icon={faAws} width='w-2/5 sm:w-1/6'>
          {`経験年数 1年\n0->1のプロジェクト開発で設定等も担当。`}
        </SkillDetail>
      </div>
    </div>
